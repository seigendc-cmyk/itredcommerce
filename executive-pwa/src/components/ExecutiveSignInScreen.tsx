...
import { fetchExecutiveRoster, signInExecutive, ExecAuthError, type RosterEntry } from '../lib/authApi';

// Forked from src/components/auth/StaffAccessScreen.tsx (confirmed loosely
// coupled to the Express flow — only the two network calls and the
// success/error shape were backend-specific). Same PIN-pad UX; the two
// network calls now hit the executive-roster/executive-signin Edge
// Functions instead of the main app's Express backend, and a real
// Supabase session (not an Express cookie) is what "authenticated" means
// here. See DL-013.

export interface ExecutiveSignInScreenProps {
  onAuthenticated: (result: { accessToken: string; refreshToken: string; staff: RosterEntry }) => void;
}

export const ExecutiveSignInScreen: React.FC<ExecutiveSignInScreenProps> = ({ onAuthenticated }) => {
  ...
  const handleAuthorize = async (e?: React.FormEvent) => {
    ...
    setErrorMessage(null);
    try {
      const result = await signInExecutive(selectedStaff.id, accessCode);
      onAuthenticated({ accessToken: result.accessToken, refreshToken: result.refreshToken, staff: result.staff as RosterEntry });
    } catch (err) {
      setAuthenticatingStaffId(null);
      if (err instanceof ExecAuthError && err.code === 'LOCKED_OUT') {
        setErrorMessage('Too many failed attempts. Please wait a moment before trying again.');
      } else if (err instanceof ExecAuthError && err.code === 'NOT_EXECUTIVE') {
        setErrorMessage('This app is only available to executive-role staff.');
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-between select-none">
      <header className="h-12 bg-[#FF6B00] text-white px-4 flex items-center justify-between shadow-sm border-b border-[#E05E00]">
        <div className="flex items-center space-x-2">
          ...
          <p className="text-xs font-semibold uppercase tracking-widest text-white/90 hidden sm:block">
            Cross-Branch Executive Dashboard
          </p>
        </div>
      </header>
          ...
          <div className="md:w-6/12 bg-slate-900 border-r border-slate-700 p-5 flex flex-col justify-between">
            <div>
              ...
              {!rosterError && staffRoster.length === 0 && (
                <div className="text-xs text-slate-500 p-2">No executive-role staff found for this tenant.</div>
              )}
      ...
      <footer className="bg-slate-900 border-t border-slate-700 px-6 py-2 text-xs text-slate-400 flex items-center justify-between font-mono">
        <div>Read-Only • Cross-Branch • Single Tenant</div>
